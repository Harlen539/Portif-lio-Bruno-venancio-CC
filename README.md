...
- `front-end/src/mobile.css`: cabeçalho compacto e botão de contato na base da primeira seção no mobile.
...
Para usar um navegador já instalado, defina `PLAYWRIGHT_CHANNEL=msedge` ou `PLAYWRIGHT_CHANNEL=chrome` no ambiente antes de rodar os testes. A suíte verifica navegação, animações, responsividade, imagens, diálogos e os controles mobile.

## Conteúdo

Edite `front-end/src/data/portfolio.js` para preencher e-mail, GitHub, WhatsApp e projetos reais. Links externos são validados e abertos com `noopener noreferrer`.

O seletor PT/EN traduz os controles de navegação e contato; o conteúdo das seções permanece em português.
